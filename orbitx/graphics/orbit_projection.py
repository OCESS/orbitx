...
class OrbitProjection:
    ...
    def __init__(self, flight_gui: FlightGui):
        ...
        self._ring = vpython.ring(
            axis=vpython.vector(0, 0, 1), opacity=1)
        ...
        self._hyperbola = vpython.curve(
            hyperbola_points, axis=vpython.vector(0, 0, 1), opacity=0)
        self._hyperbola.rotate(math.degrees(90), axis=vpython.vector(0, 1, 0))

    def update(self):
        if not self._visible:
            self._hyperbola.opacity = 0
        ...
        if e_mag < 1:
            ...
            self._hyperbola.size = vpython.vector(1, 1, 0)
