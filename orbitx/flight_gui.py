...
class FlightGui:
    ...
    def _set_caption(self) -> None:
        ...
        for caption, text_gen_func, helptext, new_section in [
            ("Orbit speed",
             lambda: f"{int(calc.orb_speed(self._reference))} m/s",
             "Speed required for circular orbit at current altitude",
             False),
            ("Periapsis",
             lambda:
                f"{int(calc.periapsis(self._reference, self._habitat))} m",
             "Lowest altitude in naïve orbit around reference",
             False),
            ("Apoapsis",
             lambda: f"{calc.apoapsis(self._reference, self._habitat)} m",
            ...
            ("HRT phase θ",
             ...
             False),
            ("Fuel ",
             lambda: f"{abs(round(self._habitat.fuel))} kg",
             "Remaining fuel of habitat",
             False),
            ("Throttle",
             lambda:
             f"{int(self._habitat.throttle):.1%}",
             "Percentage of habitat's maximum rated engines",
             False),

            ("Ref alt",
             lambda: f"{int(calc.altitude(self._reference, self._habitat))} m",
             "Altitude of habitat above reference surface",
             False),
            ("Ref speed",
             lambda: f"{int(calc.speed(self._reference, self._habitat))} m/s",
             "Speed of habitat above reference surface",
             False),
            ("Vertical speed",
             lambda:
             f"{int(calc.v_speed(self._reference, self._habitat))} m/s ",
             "Vertical speed of habitat towards/away reference surface",
             False),
            ("Horizontal speed",
             lambda:
             f"{int(calc.h_speed(self._reference, self._habitat))} m/s ",
             "Horizontal speed of habitat across reference surface",
             False),
            ("Targ alt",
             lambda:
             f"{int(calc.altitude(self._target, self._habitat))} m",
             "Altitude of habitat above reference surface",
             False),
            ("Targ speed",
             lambda:
             f"{int(calc.speed(self._target, self._habitat))} m/s",
             "...........................",
             False),
            ("Pitch",
             lambda:
             f"{int(calc.pitch(self._habitat))} m/s",
             "...........................",
             False),
            ("landing acceleration",
             lambda:
             f"{int(calc.speed(self._reference, self._habitat))} m/s",
             "..........................",
             False)
        ]:
