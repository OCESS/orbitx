...
131     CONflag = 0
        ...
        FOR i = 1 TO 241
         ...
         'if B(1,i) = 28 or B(0,i)=28 then print B(0,i),B(1,i):z$=input$(1)
...
2        IF B(1, i) = 32 AND r <= P(B(0, i), 5) + P(32, 5) THEN CONflag2 = 1: CONflag3 = B(0, i)': targ = 32
         ...
         IF B(1, i) = 38 AND r <= P(B(0, i), 5) + P(38, 5) THEN Vx(38)=Vx(B(0,i)):Vy(38)=Vy(B(0,i))
...
132     FOR i = 37 + ufo1 + ufo2 TO 0 STEP -1
         'locate 1,40:print ufo1;
         IF i = 28 THEN GOSUB 301
         'IF i = 38 THEN GOSUB 7200
         if i=38 then P(i,1)=P(i,1)+pirateTHRUSTx:P(i,2)=P(i,2)+pirateTHRUSTy  ':locate 2,30:print HSangle;
...
301     IF explosion > 1 THEN Ztel(2) = 0: Ztel(1) = 1
        IF ufo2 = 0 AND Ztel(17) = 1 AND PROBEflag = 1 THEN GOSUB 7000: PROBEflag = 0
...
461     IF cen = 40 THEN cen = 38+ufo1
        IF cen - 1 = 36 AND MODULEflag = 0 THEN cen = 36
        IF cen - 1 < 0 THEN cen = 41
        cen = cen - 1
        cenXoff = 0
        cenYoff = 0
        if cen=38 and active=0 then cen=37
        GOSUB 405
        RETURN
...
462     IF targ - 1 = 28 THEN targ = 28
        ...
        targ = targ - 1
        if targ=38 and active =0 then targ=37
        GOSUB 400
        RETURN
...
466     IF cen = 40 THEN cen = -1
        IF cen + 1 = 36 AND MODULEflag = 0 THEN cen = 36
        IF cen + 1 > 37+ufo1 THEN cen = 39
        cen = cen + 1
        if cen=38 and active = 0 then cen=0
        cenXoff = 0
        cenYoff = 0
        GOSUB 405
        RETURN
...
467     IF targ = 40 THEN targ = -1
        ...
        targ = targ + 1
        if targ=38 and active=0 then targ=39
        if targ=39 and ufo2=0 then targ=40
        GOSUB 400
        RETURN
...
701     OPEN "R", #1, filename$+".RND", 1427
        ...
        FOR i = 1 TO 39
        '    if i=37 then locate 1,30:print k;:z$=input$(1)
...
811     k=1
819     OPEN "R", #1, "ORBITSSE.RND", 279
        inpSTR$=space$(279)
        GET #1, 1, inpSTR$
        close #1
        if len(inpSTR$) <> 279 then locate 25, 1:color 12:print "ENG telem";:goto 812
        chkCHAR1$=left$(inpSTR$,1)
...
818     k = 2
        ...
        ufo1 = 1
        activeOLD = active
        active = cvi(mid$(inpSTR$,219,2))
        if activeOLD = 0 and active > 0 then active = 2
        if active = 0 and activeOLD=1 and activeAYSE=1 then AYSE=0
        if active = 0 then Px(38,3)=1000000000000000000 
        if active = 2 then Px(38,3)=cvd(mid$(inpSTR$,221,8))
        if active = 0 then Py(38,3)=1000000000000000000 
        if active = 2 then Py(38,3)=cvd(mid$(inpSTR$,229,8))
        if active = 2 then Vx(38)=cvd(mid$(inpSTR$,237,8))
        if active = 2 then Vy(38)=cvd(mid$(inpSTR$,245,8))
        pirateTHRUSTx = cvd(mid$(inpSTR$,263,8))
        pirateTHRUSTy = cvd(mid$(inpSTR$,271,8))
        'locate 1,30:print active;
        'locate 1,50:print Px(38,3);
        'locate 2,50:print Py(38,3);
        'locate 3,50:print Vx(38);
        'locate 4,50:print Vy(38);
        
        activeAYSE = cvi(mid$(inpSTR$,253,2))
        HSangle = cvd(mid$(inpSTR$,255,8))
        'locate 3,30: print HSangle;
        'locate 5,50:print activeAYSE;
        'locate 6,50:print HSangle;

        if activeAYSE=150 then Vx(33)=Vx(38):Vy(33)=Vy(38): Px(33, 3) = Px(38, 3) + (100 * SIN(HSangle)): Py(33, 3) = Py(38, 3) + (100 * COS(HSangle))
        goto 812
        ...
        IF Ztel(17) = 2 THEN ufo2 = 0: Px(39, 3) = 0: Py(39, 3) = 0: 
...
7000    'r = Ztel(19) + P(Ztel(18), 5)
        ...
        Vx(39) = Vx(Ztel(18)) + (200 * SIN(Sangle + 3.14159))
        Vy(39) = Vy(Ztel(18)) + (200 * COS(Sangle + 3.14159))
        'Px(38, 3) = Px(Ztel(18), 3) + (r * SIN(angle))
        'Py(38, 3) = Py(Ztel(18), 3) + (r * COS(angle))
        Px(39, 3) = Px(Ztel(18), 3) + (r * SIN(Sangle + 3.14159))
        Py(39, 3) = Py(Ztel(18), 3) + (r * COS(Sangle + 3.14159))
        ufo2 = 1
        z$ = ""
        RETURN
