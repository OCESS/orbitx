...
11811    IF cen * tr > 0 THEN 108        
         ...
         if i = 38 then vnSA = HSangle: gosub 128: goto 108
...
107     IF z$ = "t" THEN trail = 1 - trail: CLS : GOSUB 400
        ...
        if z$ = "Z" then active = 1 - active
        if z$ = "x" then ij=28:ii=targ:ik=1:gosub 3101
        if z$ = "y" then ij=28:ii=targ:ik=-1:gosub 3101
        if z$ = "z" then ij=28:ii=targ:ik=0:gosub 3101
        if z$ = "#" then cassiniFLAG=1-cassiniFLAG
...
301     IF Ztel(1) = 1 AND TELEupFLAG = 0 THEN Sflag = 1: MATCHacc = 0: CONSTacc = 0
        ...
        locate 1,40: color 10*active: print "ACTIVE";
...
461     IF cen = 40 THEN cen = 38+ufo1+ufo2
...
466     IF cen = 40 THEN cen = -1
        IF cen + 1 = 36 AND MODULEflag = 0 THEN cen = 36
        IF cen + 1 > 37 + ufo1 + ufo2 THEN cen = 39
        cen = cen + 1
        CLS
        cenXoff = 0
        cenYoff = 0
...
800     if REPOSITIONflag=1 then active=2*active: REPOSITIONflag=0
        outSTR$ = chr$(chkBYTE+64)
        outSTR$ = outSTR$ + mki$(active)
        if active>1 then active=1
        outSTR$ = outSTR$ + mkd$(Px(28,3))
        outSTR$ = outSTR$ + mkd$(Py(28,3))
        outSTR$ = outSTR$ + mkd$(Vx(28))
        outSTR$ = outSTR$ + mkd$(Vy(28))
'        locate 2,40:print Px(28,3);"/";cvd(mid$(outSTR$,4,8));
'        locate 3,40:print Py(28,3);"/";cvd(mid$(outSTR$,12,8));
'        locate 4,40:print Vx(28);"/";cvd(mid$(outSTR$,20,8));
'        locate 5,40:print Vy(28);"/";cvd(mid$(outSTR$,28,8));

        outSTR$ = outSTR$ + mki$(AYSE)
        outSTR$ = outSTR$ + mkd$(Sangle)
        outSTR$ = outSTR$ + mkd$(THRUSTx)
        outSTR$ = outSTR$ + mkd$(THRUSTy)
        outSTR$ = outSTR$ + chr$(chkBYTE+64)
        open "R", #1, "pirate.RND", 62
        put #1, 1, outSTR$
        CLOSE #1                
...
805     ts = cvd(mid$(inpSTR$,67,8))
        OLDts = cvd(mid$(inpSTR$,75,8))
        if filename$="OSBACKUP" then HSangle = cvs(mid$(inpSTR$,43,4)) else Sangle = cvs(mid$(inpSTR$,43,4))
        k=171
        FOR i = 1 TO 39
         if i=28 then 803
         if i=38 then 815
...
803      Px(38, 3) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Py(38, 3) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Vx(38) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Vy(38) = cvd(mid$(inpSTR$,k,8)):k=k+8
         goto 804
         
815      Px(28, 3) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Py(28, 3) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Vx(28) = cvd(mid$(inpSTR$,k,8)):k=k+8
         Vy(28) = cvd(mid$(inpSTR$,k,8)):k=k+8
...
3101    difX = Px(ii, 3) - Px(ij, 3)
        ...
        if ij=28 then REPOSITIONflag=1
        if ij=28 and ik=0 then im=0: goto 3102
        resetDIST = sqr((difX^2)+(difY^2))-P(ii,5)
        if ii=38 then resetDIST=resetDIST+10000 
        if ij=28 and resetDIST>P(ii,5)+20000 then im = P(ii,5)+15000:goto 3102
        if ij=28 and resetDIST<=P(ii,5)+20000 then im = resetDIST*.9:goto 3102
3102    GOSUB 5000
        locate 2,40:print im;
        Px(ij, 3) = Px(ii, 3) + ((P(ii, 5) + im) * SIN(angle))
        Py(ij, 3) = Py(ii, 3) + ((P(ii, 5) + im) * COS(angle))
        Vx(ij) = Vx(ii) - (ik*SIN(angle + 1.570796) * SQR(G * P(ii, 4) / (P(ii, 5) + im)))
        Vy(ij) = Vy(ii) - (ik*COS(angle + 1.570796) * SQR(G * P(ii, 4) / (P(ii, 5) + im)))
        'Px(32, 3) = Px(28, 3)
        'Py(32, 3) = Py(28, 3)
        'Vx(32) = Vx(28)
        'Vy(32) = Vy(28)
