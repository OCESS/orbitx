...
1111    CLS
        ...
        locate 7, 72:color 14,0:print chr$(186);
...
5       NEXT i
        LOCATE 7, 59: PRINT CHR$(206 - (20 * switch(50, 5)));
        ...
        LOCATE 9, 63: PRINT CHR$(206 - (switch(58, 5) * 20));
        LOCATE 9, 61: PRINT CHR$(206 - (switch(51, 5) * 20));
...
2       V = INP(889)
        ...
        Zvar#(3) = Zvar#(3) - (HABfuelleak * DELtime#)
        oldHABfuelRATE# = HABfuelRATE#
...
77      IF LOADdisp = 2 THEN PRINT "  TEMP"; : PRINT USING "####"; switch(26, 8);
        ...
        Zvar#(4) = Zvar#(4) - (AYSEfuelleak * DELtime#)
        oldAYSEfuelRATE# = AYSEfuelRATE#
        ...
        IF EL(8) < 10 THEN clr = 7 ELSE clr = 2
        LOCATE 12, 66: if LOADdisp <> 1 then COLOR 1, clr: PRINT "Network "; else color 10, clr: print " 5      ";
        LOCATE 8, 65: if LOADdisp <> 1 then COLOR 1, clr: PRINT "EECOM"; else color 10, clr: print " 10  ";
...
100     'Main HAB bus Resistors
        ...
        If Reecom > 0 THEN Rt = Rt + (1/Reecom)
        factor = 1 + ((12 AND SWITCHs%(50)) / 120) - ((48 AND SWITCHs%(50)) / 480)
        if switch(50, 5) > 0 then Rradar = 100 * factor else Rradar = 0
        IF switch(50, 5) = 1 THEN Rt = Rt + (1 / Rradar)
...
103     sht = (3072 AND SWITCHs%(19)) / 600
        IF sht = 0 THEN 104
        IF switch(19, 5) = 1 THEN Rt = Rt + (1 / (10 ^ (6 - sht)))
104     sht = (3072 AND SWITCHs%(20)) / 600
        IF sht = 0 THEN 106
        IF switch(20, 5) = 1 THEN Rt = Rt + (1 / (10 ^ (6 - sht)))
106     Rt = 1 / Rt
        RETURN
...
110     Rt1 = 1 / SHORTc(2)
        ...
        IF Rcom < .3 THEN Rcom = .3
        Rcom = 1/((1/Rcom) + .125)
        factor = 1 + ((12 AND SWITCHs%(20)) / 120) - ((48 AND SWITCHs%(20)) / 480)
        Rcom = Rcom * factor
        IF switch(20, 5) = 1 THEN Rt1 = Rt1 + (1 / Rcom) else Rcom = 0
...
820     HABengineMAX = 0
        FOR i = 1 TO 4
         IF switch(5 + (2 * i), 5) > 0 THEN c1 = EL(6) / 10000 ELSE c1 = 0
         IF switch(4 + (2 * i), 5) > 0 THEN c2 = EL(6) / 10000 ELSE c2 = 0
         IF c1 * c2 > 0 THEN HABengineMAX = HABengineMAX + (c1 * .25)
         IF c1 * c2 > 0 THEN engineOP(i) = 1 ELSE engineOP(i) = 0
        NEXT i
        HABengineMAX = HABengineMAX * ((switch(34, 5) * .5) + (switch(35, 5) * .5))
       
        AYSEengineMAX = 0
        FOR i = 1 TO 4
         IF switch(28 + i, 5) > 0 THEN AYSEengineMAX = AYSEengineMAX + (.25 * EL(9) / 100000)
        NEXT i
        COLOR 14, 0
      
        IF switch(1, 6) > 0 THEN current1 = EL(6) / switch(1, 6) ELSE current1 = 0
        IF switch(2, 6) > 0 THEN current2 = EL(6) / switch(2, 6) ELSE current2 = 0
        Zvar#(9) = (current1 + current2) / 1477000.14#
       
        IF Zvar#(13) = 150 THEN Zvar#(2) = 64000000 * AYSEengineMAX ELSE Zvar#(2) = 175000 * HABengineMAX / 4
      
        IF switch(19, 5) = 1 AND EL(6) > 8333 THEN Zvar#(10) = 1 ELSE Zvar#(10) = 0
        IF PARA = 1 THEN Zvar#(10) = Zvar#(10) + 2
        IF SRB = 1 THEN Zvar#(10) = Zvar#(10) + 4
        TV = (1 - engineOP(1)) + (1 - engineOP(2)) + (engineOP(3) - 1) + (engineOP(4) - 1)
        IF TV <> 0 THEN TV = 128 + (TV * 32): Zvar#(10) = Zvar#(10) + TV
        IF switch(65, 5) = 1 THEN Zvar#(10) = Zvar#(10) + 16
        'vernMALF = vernMALF * 16
        'Zvar#(10) = Zvar#(10) + vernMALF

        
        Zvar#(1) = SWITCHs%(54)
        IF switch(54, 5) = 0 OR EL(8) < 90 THEN Zvar#(1) = 2 + (61 AND Zvar#(1))
        Zvar#(1) = Zvar#(1) + (SWITCHs%(51) * 64)
        Zvar#(8) = DEVICEs(45)  'tsLIMIT
        IF (1 and Zvar#(26)) = 1 THEN Zvar#(8) = 1
        ...
        Zvar#(5) = 0
        Zvar#(6) = 0
        IF source(1, 2) = 1 AND HABexploded = 0 THEN Zvar#(5) = 1
        IF source(5, 2) = 1 AND AYSEexploded = 0 THEN Zvar#(6) = 1
        ...
        FOR i = 1 TO 26
         outSTR$=outSTR$+mkd$(Zvar#(i))
        NEXT i
        outSTR$ = outSTR$+chr$(chkBYTE+64)
        FOR i = 1 TO 15
         outSTR$=outSTR$+mks$(EL(i))
        NEXT i
        FOR i = 0 TO 65
         outSTR$=outSTR$+mki$(switch(i, 5))
         outSTR$=outSTR$+mks$(switch(i, 8))
         outSTR$=outSTR$+mki$(switch(i, 11))
        NEXT i
        FOR i = 1 TO 10
         outSTR$=outSTR$+mks$(coolantPUMP(i))
         FOR j = 0 TO 3
          outSTR$=outSTR$+mks$(coolant(i, j))
          outSTR$=outSTR$+mks$(RAD(i, j))
         NEXT j
        NEXT i
        outSTR$=outSTR$+chr$(chkBYTE+64)
